...
| **Una barra dibujada como un palo recto** entre sus dos nodos deformados no se dobla: una viga cuyos extremos cuelgan de nudos que casi no bajan queda recta, y una columna entre dos pisos que se corren distinto solo se **inclina** en vez de entrar en doble curvatura. No falla nada: los números son los de Python y el dibujo «funciona». Los **giros** de los nudos (`rx, ry, rz`) ya viajaban en cada desplazamiento y nadie los usaba. | La barra se dibuja con las **funciones de forma del propio elemento** (`VisorEstructura.CurvaDe`, `semana05/CONTRATO.md` §12), transcritas de la única definición en Python (`carga_movil.desplazamiento_en`). Guardia: `semana05/test_curva_deformada.py`, que además **lee las líneas del C#** y las evalúa contra la función de Python (200 tiros, peor 6.9e−17 m) para que las dos copias no diverjan en silencio. No dibuja la flecha de la carga repartida dentro del vano (viga 331 del LT2: 1.388 mm dibujados, 0.747 mm que faltan), porque sumarla sería cálculo en C# |
| **Media tabla de combinaciones sísmicas** hacía que el mapa D/C fuera optimista sin que nada fallara. Estaban las 5 de siempre, pero el sismo se mayoraba en **un solo sentido** (`+1.4EX`, `+1.4EY`) y faltaba `0.9G ± 1.4E`, que es la que manda en tracción de muros y columnas de borde. En un modelo lineal `−E` **no** es la misma demanda: da vuelta el axial sísmico, así que la pata de núcleo comprimida pasa a traccionada. Con las 6 que faltaban, los elementos que fallan pasan de **52 a 75** (+44 %). | `semana03/parametros.json` declara las **11** de NCh3171 / ACI 318-08 9.2.1, cada una con su `_por_que`. El anexo pasó de 9 a 15 casos y `semana04/exportar_unity.py` sigue cerrando su autoverificación de equilibrio en los 15. Para volver a las 5, es borrar entradas de ese JSON: nada en el código supone cuántas hay |
| **Un `u` enorme casi nunca es la capacidad: es el axial.** Y dos explicaciones que parecían obvias son falsas. (a) La rama de **tracción** de la curva P-M tiene 2 puntos, y parece poco muestreo; pero con el acero sin endurecer la interacción entre tracción pura y flexión pura **es** lineal: la recta ya es la rama nominal (comprobado al último decimal en el muro 200012). Densificarla con la corrida de fibras subiría `Mn` de 911 a 6670 kN·m, pero entero por el endurecimiento de `Steel01` hasta `ε_s = 0.10` ≈ 1.47 `fy`, que no es capacidad nominal: sería inflar. (b) Los `u = 9999` son **patas de núcleo revisadas solas**: 39 de 43 son pata de un grupo unido por brazos rígidos, y **33 pertenecen a un grupo en compresión neta**. La tracción que saca a esa pata de su curva es el par interno del núcleo. | Antes de tocar el muestreo, mirar el axial y **de qué grupo es la pata**: los grupos salen de unir por los elementos `brazo`/`brazo_rigido`. El diagnóstico entero, con sus números, en `semana05/QUE_REVISAR.md` (bloque "ESTADO AL 23-09") |
| **Una dimensión escrita como constante en el código no se parece a un supuesto, y nadie la revisa.** `benchmark_3d.py` tenía `col_b, col_h = 0.50, 0.50` (y las vigas, la losa y el `f'c`) sin `origen` ni `_supuesto` ni lámina, bajo un encabezado "MATERIAL AND SECTION DATA". **El pilar estaba mal**: la lámina 2017_67-103 rotula sus 18 pilares por piso como `P. 70x70`. Con 0.50 el cuerpo salía 38 % más flexible en Y (EY 33.16 contra 20.63 mm) y le fallaban 126 filas de columna en vez de 55; era, a la vez, la causa de "Ingeniería se deforma mucho más que el LT2" y de casi la mitad de los NO PASA. Ningún número fallaba: el equilibrio cerraba igual. | Las secciones de cada cuerpo viven en `edificios/<ed>/perfiles/*.json` con su `origen` (si se leyó del plano) o su `_supuesto: true` (si no), y el modelo las **lee** de ahí sin default: si falta una, el `KeyError` dice cuál. Al cambiar una sección cambian los **números de control** de la sección 8 y los de `AGENTS.md` y `edificios/conjunto/README.md`. Ojo con el efecto cruzado: el Ø16 de los pilares de Ingeniería era plausible en 0.50×0.50 (ρ 1.29 %) y en 0.70×0.70 es **inadmisible** (ρ 0.66 %, bajo el 1 % de ACI 318-08 10.9.1) |
...
Ingeniería `G = 52 600.50 kN`; conjunto `G = 86 749.48 kN` = la suma;
